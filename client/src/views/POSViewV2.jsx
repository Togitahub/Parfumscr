...
const POSView = ({ storeId }) => {
	...
	const products = useMemo(() => {
		const allProducts = data?.getStoreProducts ?? [];
		const query = search.trim().toLowerCase();

		return query
			? allProducts.filter(
					(storeProduct) =>
						storeProduct.product.name.toLowerCase().includes(query) ||
						storeProduct.product.brand?.name?.toLowerCase().includes(query),
				)
			: allProducts;
	}, [data, search]);
	...
	const addToCart = (storeProduct) => {
		...
		setCart((current) => {
			const existing = current.find((item) => item.productId === storeProduct.product.id);
			if (existing) {
			...
			if (saleMode === "NORMAL") {
				...
				const configureResponse = await configureOrderPurchase({
					variables: {
						...
						installmentCount:
							saleMode === "INSTALLMENTS" ? Number(installmentCount) : undefined,
						layawayDays: saleMode === "LAYAWAY" ? Number(layawayDays) : undefined,
						initialPayment:
	...
	if (done) {
		return (
			<div className="flex flex-col items-center justify-center gap-6 py-24">
				...
				>
					<BsCheckCircle className="w-8 h-8" style={{ color: "var(--color-success)" }} />
				</div>
			...
			<div className="flex flex-col gap-4">
				<div className="flex flex-col gap-1">
					...
					<p className="text-xs text-first/40">
						Selecciona los productos y define si será venta normal, a cuotas o apartado.
					</p>
				</div>
				...
				{loading ? (
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
						{products.map((storeProduct) => {
							const price = storeProduct.price ?? storeProduct.product.price;
							const stock = storeProduct.stock ?? storeProduct.product.stock ?? 0;
							const inCart = cart.find((item) => item.productId === storeProduct.product.id);
							const outOfStock = stock === 0;

							return (
								<button
									key={storeProduct.product.id}
									onClick={() => !outOfStock && addToCart(storeProduct)}
									disabled={outOfStock}
									className={[
								...
								>
									<div className="flex-1 min-w-0">
										...
										<p className="text-xs text-first/40 truncate">
											...
											{storeProduct.product.size && ` · ${storeProduct.product.size}`}
										</p>
										<div className="flex items-center justify-between mt-1">
											<span className="text-sm font-bold" style={{ color: "var(--color-second)" }}>
												{formatPrice(price)}
											</span>
											<span className="text-[10px] text-first/30">Stock: {stock}</span>
										</div>
									</div>
									{inCart && (
										<span
							...
							>
								<div className="flex-1 min-w-0">
									<p className="text-xs font-medium text-first truncate">
										{item.name}
										{item.size && <span className="text-first/35"> · {item.size}</span>}
									</p>
				...
				{cart.length > 0 && (
					<>
						...
						<div className="rounded-xl border border-first/8 p-3 flex items-center justify-between gap-3">
							<div>
								<p className="text-xs uppercase tracking-wider text-first/35">Resumen</p>
